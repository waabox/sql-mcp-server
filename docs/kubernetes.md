...
```bash
docker build -t your-registry/sql-mcp-server:v1.1.2 .
docker push your-registry/sql-mcp-server:v1.1.2
```
...
spec:
  ...
  template:
    ...
    spec:
      containers:
        - name: sql-mcp-server
          image: your-registry/sql-mcp-server:v1.1.2
          ports:
            - containerPort: 8080
              name: http
          env:
          ...
          volumeMounts:
            - name: config
              ...
              readOnly: true
          # /app/config/application.yml is loaded automatically; no `args:` needed.
          resources:
            requests:
              memory: "256Mi"
              cpu: "100m"
