...
**Local (STDIO):**
```bash
claude mcp add sql -- java -jar /path/to/sql-mcp-server-1.1.0.jar \
...
```json
{
  "mcpServers": {
    "sql": {
      ...
      "args": [
        "-jar",
        "/path/to/sql-mcp-server-1.1.0.jar",
        "--spring.config.additional-location=/path/to/application.yml"
      ]
    }
  }
...
```bash
docker pull ghcr.io/waabox/sql-mcp-server:v1.1.0   # or :latest
```
...
docker run -d --name sql-mcp \
  ...
  ghcr.io/waabox/sql-mcp-server:v1.1.0
```
